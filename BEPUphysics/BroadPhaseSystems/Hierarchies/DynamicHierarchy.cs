...
namespace BEPUphysics.BroadPhaseSystems.Hierarchies
{
    ...
    public class DynamicHierarchy : BroadPhase
    {
        ...
        /// </summary>
        private readonly int[] threadSplitOffsets = new[]
#if WINDOWS
        ...
        // WCS Edit GET INLINED DAMN IT even source codes of C# .Net library most of all NOT FUCKING INLINED DAMN IT!!!
        public int GetSplitDepth() =>
            (ParallelLooper.ThreadCount <= threadSplitOffsets.Length
                ? threadSplitOffsets[ParallelLooper.ThreadCount - 1]
                : (ParallelLooper.ThreadCount & (ParallelLooper.ThreadCount - 1)) == 0 ? 0 : 2) + (int)Math.Ceiling(Math.Log(ParallelLooper.ThreadCount, 2));
        ...
        readonly RawList<Node> multithreadingSourceNodes = new(4);
        readonly Action<int> multithreadedRefit;
        void MultithreadedRefit(int i)
            => multithreadingSourceNodes.Elements[i]?.Refit();

        readonly RawList<NodePair> multithreadingSourceOverlaps = new(10);
        readonly Action<int> multithreadedOverlap;
        ...
        readonly UnsafeResourcePool<LeafNode> leafNodes = new();
        ...
        public override void Add(BroadPhaseEntry entry)
        {
            ...
            Vector3.Subtract(ref entry.boundingBox.Max, ref entry.boundingBox.Min, out Vector3 offset);
        ...
        internal bool RemoveFast(BroadPhaseEntry entry)
        {
            ...
            try
            {
                if (root.RemoveFast(entry, out LeafNode leafNode, out root))
                {
                    leafNode.CleanUp();
        ...
        internal bool RemoveBrute(BroadPhaseEntry entry)
        {
            ...
            try
            {
                if (root.Remove(entry, out LeafNode leafNode, out root))
                {
                    leafNode.CleanUp();
                    leafNodes.GiveBack(leafNode);
                    base.Remove(entry);
