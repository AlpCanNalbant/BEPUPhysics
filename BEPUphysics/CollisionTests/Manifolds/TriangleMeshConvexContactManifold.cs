...
namespace BEPUphysics.CollisionTests.Manifolds
{
    ...
    {
        protected RawValueList<ContactSupplementData> supplementData = new(4);
        ...
        private struct BoundarySets(int sizePower)
        {
            public QuickSet<int> BlockedVertexRegions = new(BufferPools<int>.Thread, BufferPools<int>.Thread, sizePower);
            public QuickSet<Edge> BlockedEdgeRegions = new(BufferPools<Edge>.Thread, BufferPools<int>.Thread, sizePower);
            public QuickList<EdgeContact> EdgeContacts = new(BufferPools<EdgeContact>.Thread, sizePower);
            public QuickList<VertexContact> VertexContacts = new(BufferPools<VertexContact>.Thread, sizePower);
        ...
        public override void Update(float dt)
        {
            try
            {
                ...
                RigidTransform.Invert(ref convex.worldTransform, out RigidTransform inverseConvexWorldTransform);
                AffineTransform.CreateFromRigidTransform(ref inverseConvexWorldTransform, out AffineTransform convexInverseWorldTransform);
                PrecomputeTriangleTransform(ref convexInverseWorldTransform, out AffineTransform fromMeshLocalToConvexLocal);

                //Grab the convex's local space bounding box up front. This will be used for a secondary pruning step.
                convex.Shape.GetBoundingBox(ref Toolbox.RigidIdentity, out BoundingBox convexLocalBoundingBox);

                Matrix3x3.CreateFromQuaternion(ref convex.worldTransform.Orientation, out Matrix3x3 orientation);
                var guaranteedContacts = 0;
                for (int i = 0; i < triangleCount; i++)
                {
                    //Initialize the local triangle.
                    if (ConfigureLocalTriangle(i, localTriangleShape, out TriangleIndices indices))
                    {
                        ...
                        Toolbox.GetTriangleBoundingBox(ref localTriangleShape.vA, ref localTriangleShape.vB, ref localTriangleShape.vC, out BoundingBox triangleBoundingBox);

                        triangleBoundingBox.Intersects(ref convexLocalBoundingBox, out bool intersecting);
                        if (!intersecting)
                            continue;

                        //Find a pairtester for the triangle.
                        if (!activePairTesters.TryGetValue(indices, out TrianglePairTester pairTester))
                        {
                        ...
                        if (pairTester.GenerateContactCandidates(localTriangleShape, out TinyStructList<ContactData> contactList))
                        {
                            for (int j = 0; j < contactList.Count; j++)
                            {
                                contactList.Get(j, out ContactData contact);
                ...
                if (UseImprovedBoundaryHandling)
                {
                    ...
                    if (boundarySets.VertexContacts.Count == 0 && guaranteedContacts == 0 && boundarySets.EdgeContacts.Count > 1)
                    {
                        ...
                        Vector3.Dot(ref firstNormal, ref boundarySets.EdgeContacts.Elements[0].CorrectedNormal, out float dot);
                        if (Math.Abs(dot) > .01f)
                        {
                    ...
                    for (int i = 0; i < boundarySets.EdgeContacts.Count; i++)
                    {
                        ...
                        else if (boundarySets.EdgeContacts.Elements[i].ShouldCorrect || guaranteedContacts == 0)
                        {
                            ...
                            boundarySets.EdgeContacts.Elements[i].CorrectedNormal.Normalize();
                            Vector3.Dot(ref boundarySets.EdgeContacts.Elements[i].CorrectedNormal, ref boundarySets.EdgeContacts.Elements[i].ContactData.Normal, out float dot);
                            boundarySets.EdgeContacts.Elements[i].ContactData.Normal = boundarySets.EdgeContacts.Elements[i].CorrectedNormal;
                    ...
                    for (int i = 0; i < boundarySets.VertexContacts.Count; i++)
                    {
                        ...
                        else if (boundarySets.VertexContacts.Elements[i].ShouldCorrect || guaranteedContacts == 0)
                        {
                            ...
                            Vector3.Dot(ref boundarySets.VertexContacts.Elements[i].CorrectedNormal, ref boundarySets.VertexContacts.Elements[i].ContactData.Normal, out float dot);
        ...
        protected void GetNormal(ref Vector3 uncorrectedNormal, TriangleShape localTriangleShape, out Vector3 normal)
        {
            ...
            Vector3.Subtract(ref localTriangleShape.vB, ref localTriangleShape.vA, out Vector3 AB);
            Vector3.Subtract(ref localTriangleShape.vC, ref localTriangleShape.vA, out Vector3 AC);
        ...
        public struct TriangleIndices : IEquatable<TriangleIndices>
        {
            ...
            public readonly bool Equals(TriangleIndices other)
